...
interface AddCustomTermModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddTerm: (termEn: string, termZh: string, categoryId: string) => void;
...
export const AddCustomTermModal: React.FC<AddCustomTermModalProps> = ({
  ...
  const handleTranslateToEnglish = async () => {
    ...
    try {
      ...
      if (translation) {
        setTermEn(translation);
      } else {
        setTranslateEnError('翻譯失敗或未返回結果。');
      }
    } catch (e: any) {
  ...
  const handleTranslateToChinese = async () => {
    ...
    try {
      ...
      if (translation) {
        setTermZh(translation);
      } else {
        setTranslateZhError('翻譯失敗或未返回結果。');
      }
    } catch (e: any) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    onAddTerm(termEn, termZh, categoryId);
    onClose(); 
  };
          ...
          <div>
            <div className="flex justify-between items-end mb-1">
              <label htmlFor="termZh" className="block text-sm font-medium text-gray-700">
                中文詞語 <span className="text-red-500">*</span>
              </label>
              <button
                type="button"
          ...
          <div>
            <div className="flex justify-between items-end mb-1">
              <label htmlFor="termEn" className="block text-sm font-medium text-gray-700">
                英文詞語 <span className="text-red-500">*</span>
              </label>
              <button
                type="button"
